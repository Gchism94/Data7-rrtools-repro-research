...
### Launch [Rstudio Cloud](https://login.rstudio.cloud/)

-   Log in with **GitHub**

-   Click on **RStudio Cloud**
...
Installing `rrtools` **imports many of the packages we'll need today** (e.g., have a look at the imports section of the [`DESCRIPTION`](https://github.com/benmarwick/rrtools/blob/master/DESCRIPTION) file).
...
To **create `rrcompendium` without a directory you can use:**

```{r, eval = FALSE}}
rrtools::use_compendium("rrcompendium")
```

Go ahead and **create a compendium at a location of your choice**. Stick with compendium name `rrcompendium` for ease of following the materials. If the call was successful you should see the following console output:

    ✔ Setting active project to '/path/'
    ✔ Creating 'R/'
    ✔ Creating 'man/'
    ✔ Writing 'DESCRIPTION'
    ✔ Writing 'NAMESPACE'
...
We can initialize our compendium with `.git` using:

```{r}
usethis::use_git()
```

**Beware, you may have ended up with two Rstudio sessions of `rrcompendium`.** Make sure to only have one session of a single project at one time to avoid problems.
...
`rrtools::use_compendium()` creates the **bare backbone of infrastructure required for a research compendium**. At this point it provides facilities to store general metadata about our compendium (e.g., bibliographic details to create a citation) and manage dependencies in the `DESCRIPTION` file and store and document functions in the `R/` folder. Together these allow us to **manage, install and share functionality associated with our project**.
...
Finally, let's **add a license for the material we create**. We'll use an [GPLv3 License](https://www.gnu.org/licenses/gpl-3.0.en.html). We can do this with:

```{r}
 usethis::use_gpl3_license(copyright_holder = "My Name")
```

    ✔ Setting License field in DESCRIPTION to 'gpl3 + file LICENSE'
    ✔ Writing 'LICENSE.md'
...
Let's **adjust the data LICENSE to match the source compendium license**, which is CC-BY 4.0.
